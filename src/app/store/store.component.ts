import {
  Inject, Component, OnInit, OnDestroy, Input, Output,
  ChangeDetectorRef, OnChanges, EventEmitter, ElementRef, HostListener, ViewChild, AfterViewInit
} from '@angular/core';
...
export class StoreComponent implements OnInit, OnDestroy {
  ...
  constructor(public el: ElementRef,
    ...
    @Inject(Router) router: Router, private winRef: WindowRef) {
  ...
  protected assignActity(type: string): void {
    const newWindow = this.nativeWindow.open('/#/link');


    newWindow.location = '/#/link/' + type;
    console.log(type);
  }
  ...
  ngOnInit() {
    ...
    let numberOfItem = 0;
    console.log({cartConfig :this.cartConfig});
      ...
      this.products$.subscribe(
        products => {
          ...
          products.forEach(function(item, index) {
            const datalayerProduct = {'brand': 'Expert-PDF', 'category': 'lp-dynamic-' + item.Culture,
      ...
      'eventCallback': function() {
       ...
       // load(endPoint);
        // window.open(endPoint, '_blank');
         // Object.assign(document.createElement('a'), { target: '_self', href: endPoint}).click();
        // Window.open('http://www.c-sharpcorner.com/1/302/angularjs.aspx', 'C-Sharpcorner', 'width=500,height=400');
         _router.navigate(['/']).then(result =>  {window.location.href = endPoint; });
